...
export default function DeckCard({ song, isPlaying, onTogglePlay, onNext, onPrev, isBackground = false }: CardProps) {
  return (
    <div className={`relative w-full h-full bg-white rounded-[24px] overflow-hidden flex flex-col border border-zinc-200 transition-all ${isBackground ? 'brightness-95 grayscale-[0.3] shadow-none' : 'shadow-[0_8px_30px_rgba(0,0,0,0.1)] ring-1 ring-black/5'}`}>
      ...
      {/* Card Content */}
      <div className="flex-none px-6 flex flex-col relative z-10 pt-6 justify-start">
        <motion.h1 
      ...
      {/* Large Unsplash Cover Art just above the player */}
      <div className="flex-1 px-4 py-2 relative z-10 flex flex-col justify-center min-h-0">
        <div className="w-full h-full rounded-2xl overflow-hidden border border-zinc-200/60 shadow-sm relative group/cover bg-zinc-100">
          <img 
            src={song.cover} 
            alt={song.title} 
            loading="lazy"
