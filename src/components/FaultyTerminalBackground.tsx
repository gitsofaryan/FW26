...
export const FaultyTerminalBackground = React.memo(function FaultyTerminalBackground({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
          value: new Float32Array([gl.canvas.width, gl.canvas.height, gl.canvas.width / (gl.canvas.height || 1)])
        },
        uScale: { value: scale },
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full relative overflow-hidden bg-[#0b0c10] ${className}`}
      style={style}
      {...rest}
    />
  );
